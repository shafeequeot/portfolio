  ...
  const projectList = portfolio.webProjects.map((key, id) =>
    <div id='whatIKnow' className='bg-dark-800 p-2 rounded flex ' >
      ...
      <div class="group cursor-pointer flex flex-col">
        ...
        <Image width={330} height={220} loader={imageLoader} className='object-fill' src={key.image} alt={key.name}></Image>
        </div>
        ...
        <Image width={700} height={500} loader={imageLoader} className='w-full' src={key.image} alt={key.name}></Image>
        </div>
      </div>
